...
show_swap_usage(struct task_context *tc, ulong exists, ulong flag) 
{
	...
	// Do not display the the pid's swap usage is zero
	if ((flag & DISPLAY_NON_ZERO) && swap_usage == 0)
		return;
...
cmd_pswap(void)
{
	...
	while ((c = getopt(argcnt, args, "kGz")) != EOF) {
		switch (c) {
                case 'k':
                        flag |= DISPLAY_KB;
                        break;
