...
enum e_instructions {
    MOVZBL, MOVZWL, MOVSLQ, MOVSBL, MOVABS, MOV, CMOVcc, LEA, 
    PUSHF, PUSH, POPF, POP, CALL, RET, LEAVE, JMP, Jcc,
    BTS, BTR, BT, CMP, TEST, INC, DEC, 
    SHL, SHR, SAR,  IMUL, SUB,
    SBB, XADD, ADD, NOT, AND, OR, XOR,
    UD2, NOP,
    ICOUNT
};
...
const char *s_instructions[] = {
    "movzbl", "movzwl", "movslq", "movsbl", "movabs", "mov", "cmov", "lea", 
    "pushf", "push", "popf", "pop", "call", "ret", "leave", "jmp", "j",
    "bts", "btr", "bt", "cmp", "test", "inc", "dec", 
    "shl", "shr", "sar", "imul", "sub", 
    "sbb", "xadd", "add", "not", "and", "or", "xor", 
    "ud2", "nop", 
...
uint8_t step_into_symbol(char *s) {
    const char *calling_symbol[] = { "error_entry", "irq_to_desc", 
        "save_args", "save_paranoid", "__get_user_" };
    int i;
    for (i = 0; i < sizeof(calling_symbol) / sizeof(char *); i++)
        if (STRNEQ(s, calling_symbol[i]))
            return 1;
    return 0;
}
...
static int disassemble_frame(struct stack_frame_t *, unsigned char, uint8_t);
...
static uint8_t is_apic_interrupt(const char *s) {
    const unsigned char lens[] = {
        15, 17, 18, 19, 20, 20, 20, 23,
    };

    const char *ai[] = {
        "error_interrupt", "thermal_interrupt", "spurious_interrupt",
        "threshold_interrupt", "reschedule_interrupt", "invalidate_interrupt",
        "apic_timer_interrupt", "call_function_interrupt", 0
    };

    int16_t i = 0, j, l = strlen(s);
    if (!(s && *s))
        return 0;
...
static uint64_t get_frame_size(struct stack_parser_context *ctx, char *symbol, uint64_t rip, uint8_t may_return) {
    ...
    if (disassemble_frame(&frame, may_return, 0))
        return (uint64_t)-1;
...
static uint8_t fill_frames(struct bt_info *bt, struct stack_parser_context *ctx) {
    uint64_t frame_stack_rip_address, frame_stack_rip, frame_size;
...
//    uint64_t i;
    ...
    // We're going to find the first (innermost) frame.
    while (current_frame_end < bt->stacktop) {
        readmem(current_frame_end, KVADDR, &frame_rbp_rip, 16, "frame_rbp_rip", RETURN_ON_ERROR | QUIET);
        if (is_kernel_text(frame_rbp_rip.rip) && is_kernel_data(frame_rbp_rip.rbp))
            break;
...
//        if (CRASHDEBUG(4))
        ...
        if (disassemble_frame(p_cframe, may_return, 0))
            return 1;
        ...
        if (-1 == except_no) {
             ...
             */
            frame_size = get_frame_size(ctx, p_cframe->symbol, frame_stack_rip, exception_encountered ? 1 : may_return);
            if (frame_size == (uint64_t)-1)
                return 1;

            if (exception_encountered)
                current_frame_start = current_frame_start + frame_size - 0x10;
            else if (calculate_frame_start)
                current_frame_start = current_frame_end + frame_size;
...
//            char *p;
            ...
            p_cframe->rsp = ((p_cframe - 1)->rsp | 0x1fff) + 1 - 0x28;
...
static int disassemble_frame(struct stack_frame_t *frame, unsigned char will_ret, uint8_t recursive) {
#define MIN_CODE_BUFFER     1024
    ...
    if (!try_disassemble(frame->symbol, frame->rip)) {
        fprintf(fp, "Can't disassemble symbol '%s' at 0x%lx\n", frame->symbol, frame->rip);
        fprintf(fp, "Load corresponding module and try again\n");
        return 1;
    }
...
#define third   (frame->code[line_i].third)
    ...
    for(line_i = 0; line_i < frame->len; ) {
        ...
        switch(instr_i) {
            ...
            case MOVZWL:
            case CMOVcc:
                // Look at the end of `case MOV`. If dest register is callee-save,
                // then we'll remove created 'callee-save => parameter' mapping.
                if (dreg_i < RCOUNT)
                ...
                if (sreg_i == GS_REG) {
                    // Omit the instructions like:
                    // mov    %gs:0xb7b8,%rsp
                    // within `system_call`
                    if (is_stack_register(dreg_i) && STREQ(frame->symbol, "system_call"))
                        break;
            ...
            case CALL:
                if (first && *first) {
                    ...
                    if (may_call && calling_symbol && step_into_symbol(calling_symbol)) {
...
//                        print_value(regs, RSP);
            ...
            case TEST:
                unreliable_flags = 1;
                if (INVALID != sreg_i && RELIABLE_NO == get_reliable_state(regs, sreg_i) == RELIABLE_NO)
                    break;   // Unreliable;
...
static struct command_table_entry cmd_tbl[] = {
...
void __attribute__((constructor)) fp_init(void) { register_extension(cmd_tbl); }
	...
	ulong value;
        ...
        for (i = 1; ; i++) {
                ...
                if (i == 1 && argcnt == 1)
                        ...
                        str_to_context(args[i], &value, &(bt.tc));
                if (!bt.tc)
                        continue;
                bt.task = bt.tc->task;
                ...
                bt.stkptr = rsp;
                parse_stack(&bt);
                if ((argcnt == 1) || (argcnt > 1 && i + 1 == argcnt))
                        break;
        }
}
